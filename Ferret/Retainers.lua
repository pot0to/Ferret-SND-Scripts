...
function Retainers:ready()
    return ARRetainersWaitingToBeProcessed()
end

function Retainers:close()
    yield("/callback RetainerList true -1")
end

function Retainers:check()
    if not self.enabled or not self:ready() then
        return
    end
...
function Retainers:reset()
    self.ferret.logger:warn('No reset script provided.')
end